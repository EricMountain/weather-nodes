...
{
public:
    SunAndMoon(int year, int month, int day, int hour, int minute, int second, double latitude, double longitude, int utc_offset_seconds)
        : year(year), month(month), day(day), hour(hour), minute(minute), second(second), latitude(latitude), longitude(longitude), utc_offset_seconds(utc_offset_seconds),
          sunMoonCalc(year, month, day, hour, minute, second, latitude, longitude)
    {
    ...
    double getMoonPhaseAge()
    {
        return sunMoonCalcResult.moon.age;
    }
